...
// ===== 本地离线识别（Vosk，WASM）=====
// 模型已随扩展打包（models/*.tar.gz），加载后完全离线、无需 Key、无需网络。
...
async function startVosk() {
  ...
  if (typeof globalThis.Vosk === "undefined") {
    throw new Error("本地识别组件未加载（vosk.js 缺失），请重新加载扩展。");
  }
  if (!voskModel) {
    const modelUrl = chrome.runtime.getURL(modelFile);
    voskModel = await globalThis.Vosk.createModel(modelUrl);
  }
  voskRecognizer = voskModel.registerRecognizer({ sampleRate: 16_000 });
...
function stopVoskRecognizer() {
  if (voskRecognizer) {
    try { voskRecognizer.remove(); } catch { /* ignore */ }
    voskRecognizer = null;
  }
  if (voskModel) {
