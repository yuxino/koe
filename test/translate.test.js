...
test("translates lines in order across batched requests", async () => {
  ...
  const lines = await translateLines({
    ...
    fetchImpl: async (_url, options) => {
      ...
      const userContent = body.input.messages.find((message) => message.role === "user")?.content || "";
      calls.push(userContent);
      const numbered = userContent.match(/\d+\. [^\n]+/g) || [];
...
test("marks lines whose translation equals the original as translated", async () => {
  const lines = await translateLines({
    lines: [
      { startMs: 0, endMs: 1_000, text: "哦" },
      { startMs: 1_000, endMs: 2_000, text: "你好" }
    ],
    apiKey: "test-key",
    fetchImpl: async () => ({
      ok: true,
      json: async () => ({
        output: { choices: [{ message: { content: "1. 哦\n2. 你好" } }] }
      })
    })
  });
  assert.equal(lines[0].translated, "哦");
  assert.equal(lines[1].translated, "你好");
});
