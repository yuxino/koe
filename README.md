...
- **Offline recognition** — bundled Vosk models in `models/`; once loaded, subtitles keep working without any network, and microphone mode needs no gesture and no key.
...
Main pieces: `background.js` coordinates sessions, state, and the subtitle record; `offscreen.js` captures audio and runs recognition (DashScope / Chrome built-in / bundled Vosk models); `content.js` detects videos and shows status toasts; `popup.*` is the gesture-based start surface; `sidepanel.*` renders the side panel and scrolling subtitle feed; `vosk.js` / `vosk-worker.js` / `models/` power offline recognition.
