(() => {
  ...
  shadow.innerHTML = `
    <style>
      :host { all: initial; }
      .stage { position: fixed; left: 50%; bottom: 3vh; transform: translate(-50%, 12px); opacity: 0; transition: opacity .28s ease, transform .28s ease; max-width: min(820px, 78vw); text-align: center; color: #fbf4df; font-family: Georgia, 'Songti SC', serif; }
      ...
      .translated { margin: 0; font-size: clamp(8.5px, 1vw, 12px); line-height: 1.3; letter-spacing: .02em; text-shadow: 0 2px 14px rgba(0, 0, 0, .4); }
      .original { margin: 5px 0 0; font-size: clamp(6.5px, 0.75vw, 8.5px); line-height: 1.25; color: rgba(251, 244, 223, .78); letter-spacing: .02em; text-shadow: 0 1px 10px rgba(0, 0, 0, .45); }
      ...
      .stage.compact .translated { font-size: 10px; }
